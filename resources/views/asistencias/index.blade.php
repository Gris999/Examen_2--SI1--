...
<div class="d-flex justify-content-between align-items-center mb-3">
  <div>
    ...
    <div class="text-muted small mt-1">Total de asistencias: {{ $asistencias->total() }}</div>
  </div>
  
  <div class="d-flex gap-2">
    @if(auth()->check() && auth()->user()->roles()->whereIn('nombre', ['administrador','admin','coordinador','decano'])->exists())
      <a href="{{ route('historial.index') }}" class="btn btn-outline-secondary">Ver Historial</a>
    @endif
    @if(auth()->check() && auth()->user()->roles()->whereIn('nombre', ['administrador','admin','coordinador','docente'])->exists())
...
<div class="card shadow-sm border-0 mb-3">
  <div class="card-body">
    <form method="GET" class="row g-2 align-items-center m-0">
      ...
      @if(! (auth()->check() && auth()->user()->roles()->where('nombre','docente')->exists()))
        <div class="col-md-3">
